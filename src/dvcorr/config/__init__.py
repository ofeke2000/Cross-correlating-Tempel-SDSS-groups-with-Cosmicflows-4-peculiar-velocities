...
`selection.py`), aggregated by `Settings` (`settings.py`). `shells.py` also
carries the pure shell-edge and volume-weighted-abscissa builders
(`linear_shell_edges`, `log_shell_edges`, `volume_weighted_shell_radii`) that
back `ShellConfig`'s properties. Re-exported here for a clean import surface:
`from dvcorr.config import Settings, default_settings, PathsConfig,
CosmologyConfig, ShellConfig, SelectionConfig, SPACING_LINEAR, SPACING_LOG,
