...
normalisation of the final science number. Change these only deliberately,
...
Pair separation vector.
    ...
    centre; V is the neighbour. Every pair in the project is oriented this way,
...
Angular cosine.
    ...
    sight of the CENTRAL (density) object, not of the neighbour. mu is therefore
    a group-centred cosine: mu = +1 puts the neighbour directly behind the
    centre as seen by the observer, mu = -1 puts it directly in front.
...
Sign convention (the one that bites).
    ...
    What is negative over the WHOLE shell is the product:
      ...
      - Far side (mu > 0): the neighbour lies beyond the centre and falls back
        toward it, i.e. toward the observer, so its observer-centred radial
        velocity is u < 0. Product u*mu < 0.
      - Near side (mu < 0): the neighbour lies in front of the centre and falls
    ...
    centre, the infall profile v_inf(r) is itself negative, and
    ...
    Note on the literature: Nusser (2017) centres on the VELOCITY object and
...
Observer position.
    A single module-level constant, OBSERVER_POSITION, default = box centre.
    There is exactly one observer per analysis run. Everything observer-centred
...
Periodic boundary conditions.
    ...
    every spatial calculation -- separations, neighbour queries, lines of sight
...
#: Default is the box centre. Read-only: n_hat_T and u are defined relative to
...
#: Factor relating this project's group-centred multipoles to the
#: velocity-centred multipoles of Nusser (2017): xi_Tu,ell = (-1)^ell xi_uT,ell.
...
#: Code addresses halo properties through these keys only; a catalogue with
